...
interface ChatStore {
  messages: ChatMessage[]; currentQuestionIndex: number; userName: string; isStreaming: boolean; isComplete: boolean
  addUserMessage: (text: string) => void; startBotMessage: () => void; appendToken: (token: string) => void; finaliseBotMessage: () => void
  setUserName: (name: string) => void; advanceQuestion: () => void; markComplete: () => void; reset: () => void
}

const initialMessages: ChatMessage[] = [{ role: 'bot', content: welcomeMessage }, { role: 'bot', content: firstQuestion }]

export const useChatStore = create<ChatStore>((set) => ({
  messages: initialMessages, currentQuestionIndex: 0, userName: '', isStreaming: false, isComplete: false,
  ...
  reset: () => set({ messages: initialMessages, currentQuestionIndex: 0, userName: '', isStreaming: false, isComplete: false }),
}))
